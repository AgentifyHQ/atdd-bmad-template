...
### Design Assets (Figma exports, mockups, screenshots)
- Location: `features/{layer}/{domain}/assets/`
- Example: `features/acceptance/user-management/assets/login-page.png`
- Supported formats: PNG, JPG, SVG, WebP, GIF
- The build script auto-detects `assets/` directories and embeds images as a scrollable gallery on the living docs feature page
- Use descriptive filenames — `login-page.png` renders as label "Login Page"
- Images are cache-busted with content hash (`?v=a3f2b1c4`) — no stale browser cache
- These serve as design spec pre-development, then can be supplemented by Playwright visual baselines

### Visual Regression Baselines
- Location: `tests/{testDir}/__snapshots__/{name}.png` (auto-generated by Playwright)
- Created on first run of `@visual` scenarios, committed to git
- Update after intentional changes: `npx playwright test --update-snapshots --grep @visual`
...
| Add a visual regression test | `features/acceptance/{domain}/{story}-visual.feature` with `@visual` tag. Steps already exist in `visual-regression.steps.ts` |
| Add design mockups | `features/{layer}/{domain}/assets/{name}.png` — auto-embedded in living docs |
